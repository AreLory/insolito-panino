...
export default function ProductCard({ item }: Props) {
  ...
  return (
    <div className={`border rounded shadow flex flex-col p-2 max-w-46 ${ customIng ? '': 'max-h-70'} `}>
      ...
      <div className="flex flex-col items-center p-1">
        <h3 className="text-sm md:text-xl font-bold mt-2 w-full text-center">
          {item.name}
        </h3>
        {item.sizes && (
          <div className="flex gap-2 mt-2 w-full justify-around">
            {item.sizes.map((size) => (
              <button
                ...
                className={`px-2 py-1 border rounded cursor-pointer  h-10 text-[10px] md:text-sm ${
        ...
        <div className="flex justify-between w-full items-center mt-1">
          ...
          { cartItem ? (
            ...
            // ! Undefined => Login First message 
            <button
              onClick={isAuthenticated ? handleAddToCart : undefined}
              className={`mt-3 p-1 text-[10px] w-[50%] py-2 rounded transition hover:scale-105 cursor-pointer ${isAuthenticated ? ' bg-shade text-white':'bg-gray-500 text-white'}`}
            >
              Add to Cart
            </button>
          )}
